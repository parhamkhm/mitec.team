...
| `index.html` → Work tiles | Alt texts «صفحه‌ی اصلی سایت کافه مری» / «صفحه‌ی اصلی فروشگاه تجهیزات پزشکی کارآمد»; E2's frame «به‌زودی», named «تصویر سایت کافه E2 به‌زودی» for screen readers | New with the screenshots; not in `copy-final.md` (the alt texts come from the request, the E2 name was written to match). | Owner to confirm. |
