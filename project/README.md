...
معرفی می‌گذرید و به اتاق روشن Proof با آمار واقعی می‌رسید)، نمونه‌کارها، خدمات (شش کارت)، فرایند،
درباره‌ی ما، برآورد سریع، سؤالات متداول و باند پایانی. بخش نظر مشتری‌ها در markup هست ولی تا رسیدن نظرهای
واقعی پنهان (`hidden`) است.
...
- `src/scripts/scope.js` — برآورد سریع (قیمت و زمان): همه‌چیز — بسته‌ها، موردها، قیمت‌ها، روزها و متن‌ها — از
  ...
  خلاصه (پایه + جمع + دکمه) در دسکتاپ همیشه sticky است و هیچ‌وقت از صفحه بلندتر نمی‌شود (فقط فهرست پایه داخلش اسکرول
  می‌خورد)، و افزونه‌ها کاشی‌های هم‌ارتفاع‌اند: سه‌تایی از ۱۲۸۰ پیکسل، دوتایی تا آن‌جا، و فهرست بلند بعد از ۹ (زیر ۱۰۲۴
  پیکسل ۶) تا می‌شود (DESIGN.md §5، Forms).
...
- اعداد آمار در `src/data/site-copy.json` — فقط واقعیت، بدون عدد ساختگی. عدد «پروژه‌ی تحویل‌شده» خودکار از تعداد
  پروژه‌های `src/data/portfolio.json` می‌آید؛ هر پروژه‌ی جدید هم آن‌جا ثبت شود و هم ردیفش در بخش نمونه‌کارهای `index.html`.
